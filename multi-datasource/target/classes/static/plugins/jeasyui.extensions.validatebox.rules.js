...
(function () {
    ...
    $.extend($.fn.validatebox.extensions, {
        ...
        rules: {
            ...
            notpleaseText: {
                validator: function (value,param) { 
                	var text = param?param:$(this).parent().prev().attr("nullField");
                	if(value==text || !value){
                		return false;
                	}
                	return true;
            ...
            passEqualTo: {
                validator:function(value,param){
                	var f = $(param[0]).val() == value;
                	var errMsg = '两次输入的密码不一致';
            		if(f){
            			$(this).validatebox('options').isValid=true;
            			$(param[0]).next("span.textbox").find(".textbox-text").validatebox('options').isValid=1;
            			$(param[0]).next("span.textbox").find(".textbox-text").validatebox('options').forceErrMsg='';
            			$(param[0]).next("span.textbox").removeClass("textbox-invalid");
            		}else{
            			$(this).validatebox('options').isValid=false;
            			$(this).validatebox('options').forceErrMsg=errMsg;
            			$(param[0]).next("span.textbox").find(".textbox-text").validatebox('options').isValid=0;
            			$(param[0]).next("span.textbox").find(".textbox-text").validatebox('options').forceErrMsg=errMsg;
            			$(param[0]).next("span.textbox").addClass("textbox-invalid");
            		}
            		return f;
                },
                message:'两次输入的密码不一致'
